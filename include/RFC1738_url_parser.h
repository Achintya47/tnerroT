...
/**
 * @brief Splits a "udp://host:port" announce URL (BEP 15) into host + port.
 *        No path/query is involved in the UDP tracker protocol, so unlike
 *        parse_announce_url() there is no path_out.
 *
 * @param url        Null-terminated announce URL (as read from the .torrent).
 * @param host_out   Receives a malloc'd, null-terminated hostname. Caller frees.
 * @param port_out   Receives the port.
 * @return 1 on success, 0 on malformed input or if the scheme isn't "udp://".
 */
